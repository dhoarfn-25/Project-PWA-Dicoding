...
async function getPertandingan(id) {
if (id !== null) {
    return await getPertandinganTim(id, false)
} else {
    try {
        if ('caches' in window) {
            let res = await caches.match(endPoint.fixture)
            return await res.json()
        }
    } catch (error) {
        try {
            const res = await fetchData(endPoint.fixture)
            return await res.json()
        } catch (error) {
            console.log(error);
        }
